	...
	infra.template={
			...
			if(search){
				...
				if(search){
					try{
						...
						var r=sequence.get(data,dataroot);
		...
		getVar:function(conf,v){
			...
			if(v==undefined){
				...
				var p=sequence.right(conf['dataroot'].concat(right));
				...
				if(p[p.length-1]=='$key'){
					...
					var value=sequence.get(conf['data'],p);//Относительный путь, от данных
					...
					if(typeof(value)=='undefined'&&right.length){//Абсолютный путь, от scope
						value=sequence.get(scope,right);
						if(typeof(value)!=='undefined')root=right;
					}
					if(typeof(value)=='undefined')root=right;
				}
			...
			for(var i=0,l=v.length;i<l;i++){
				...
				if(start&&sym===']'){
					...
					if(sym==':'){
						...
						if(str) res=res.concat(sequence.right(str));
				...
				if(sym==='['){
					if(start){
						...
						res=res.concat(sequence.right(str));
						start=true;
					}
				}
				if(!start)str+=sym;
